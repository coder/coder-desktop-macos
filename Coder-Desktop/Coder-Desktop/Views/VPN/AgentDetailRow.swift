...
///
/// `leadingIcon` is rendered ahead of the status dot — used to mark sub-agent
/// rows with a cube glyph so they're identifiable without a dedicated
/// container header. `appsToggle`, when set, exposes the show/hide control
/// for parent apps (mirrors the dashboard's "Show parent apps" affordance).
